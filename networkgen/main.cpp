...
int main(int argc, char *argv[]) {
    ...
    if (generate) {
        ...
        if(Finalize_iterative(innerNetwork) != 0){
...
int Finalize_iterative(PoreNetwork * pn){
    ...
    for(int dir = 0; dir <= 2; dir++){
        if(pn->ns->flowDirs[dir]){
            ...
            size_t lengthTL = P_Bound->generateFullConnectivity();
//            size_t lengthTL2 = P_Bound->generateFullConnectivity_NearlySorted();

//            //CHECK
//            if(lengthTL != lengthTL2){
//                std::cout << "Length Differs!!" << std::endl;
//                std::abort();
//            }
//            for(size_t i = 0; i <lengthTL; i++) {
//                if (P_Bound->throatList_full[0][i] != P_Bound->throatList_full2[0][i] &&
//                    P_Bound->throatList_full[1][i] != P_Bound->throatList_full2[1][i]) {
//                    std::cout << "!-";
//                }
////                std::cout << '[' << i << "]\t"
////                << P_Bound->throatList_full[0][i] << '\t' << P_Bound->throatList_full[1][i] << ":\t"
////                << P_Bound->throatList_full2[0][i] << '\t' << P_Bound->throatList_full2[1][i] << std::endl;
//
//            }
            ...
            char * pb_list = searchForIsolatedPB_iterative(P_Bound, lengthTL);
